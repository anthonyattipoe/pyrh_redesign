...
class Order(object):
    ...
    def place(self) -> OrderStatus:
        ...
        if self.order_type == Order.Type.BUY:
            ...
            response = self.rh.place_market_buy_order(self.instrument.order_object(), self.instrument.ticker_symbol, time_in_force, self.quantity)
        elif self.order_type == Order.Type.MARKET_SELL_ORDER:
            response = self.rh.place_market_sell_order(self.instrument.order_object(), self.instrument.ticker_symbol, time_in_force, self.quantity)
        elif self.order_type == Order.Type.STOP_LIMIT_BUY_ORDER:
            response = self.rh.place_stop_limit_buy_order(self.instrument.order_object(), self.instrument.ticker_symbol, time_in_force, self.price, self.stop_price, self.quantity)
        elif self.order_type == Order.Type.STOP_LIMIT_SELL_ORDER:
            response = self.rh.place_stop_limit_sell_order(self.instrument.order_object(), self.instrument.ticker_symbol, time_in_force, self.price, self.stop_price, self.quantity)
        elif self.order_type == Order.Type.STOP_LOSS_BUY_ORDER:
            response = self.rh.place_stop_loss_buy_order(self.instrument.order_object(), self.instrument.ticker_symbol, time_in_force, self.stop_price, self.quantity)
        elif self.order_type == Order.Type.STOP_LOSS_SELL_ORDER:
            response = self.rh.place_stop_loss_sell_order(self.instrument.order_object(), self.instrument.ticker_symbol, time_in_force, self.stop_price, self.quantity)
